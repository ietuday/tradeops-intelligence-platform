...
func TestProcessWithRetryPublishesDLQAfterExhaustion(t *testing.T) {
	writer := &fakeWriter{}
	processor := &fakeProcessor{err: errors.New("invalid order event")}
	consumer := &Consumer{
		service:     processor,
	...
	err := consumer.processWithRetry(context.Background(), kafka.Message{
		Topic: "order.filled",
		Key:   []byte("order-1"),
		Value: []byte(`{"bad":true}`),
	...
	if event.OriginalTopic != "order.filled" || event.ServiceName != "portfolio-service" || event.RetryCount != 2 || event.CorrelationID != "corr-1" {
...
func (p *fakeProcessor) ProcessOrderFilled(context.Context, []byte) error {
	p.calls++
	return p.err
}
