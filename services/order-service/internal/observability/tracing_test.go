...
func TestRoutePatternWithChiContext(t *testing.T) {
	...
	router.ServeHTTP(httptest.NewRecorder(), httptest.NewRequest(http.MethodGet, "/orders/123", nil))

	if got != "/orders/{id}" {
		t.Fatalf("routePattern with chi context = %q, want /orders/{id}", got)
	}
}

func TestTracingHandlersDoNotPanicWithoutChiRouteContext(t *testing.T) {
	handler := HTTPHandler("order-service", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	handler.ServeHTTP(httptest.NewRecorder(), httptest.NewRequest(http.MethodGet, "/health", nil))

	traced := TraceAttributes("order-service")(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
