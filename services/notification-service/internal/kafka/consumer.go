...
func NewConsumer(brokers []string, topics []string, svc Processor, logger *slog.Logger, metrics *observability.Metrics, retryConfig RetryConfig) *Consumer {
	...
	return &Consumer{
		...
		logger:      logger,
		metrics:     metrics,
		dlqWriter:   newDLQWriter(brokers, "notification.dlq"),
...
func (c *Consumer) consume(ctx context.Context, reader readerCloser) {
	for {
		...
		if err != nil {
			...
			c.logger.Warn("failed to fetch notification source event", "error", err)
			continue
		}
		...
		processCtx, span := observability.StartConsumerSpan(processCtx, "notification-service", event.Topic, event.CorrelationID, payloadField(event.Value, "tenantId"))
		if err := c.processWithRetry(processCtx, event); err != nil {
			c.logger.Warn("failed to process notification source event", "topic", message.Topic, "partition", message.Partition, "offset", message.Offset, "error", err)
		}
		span.End()
		if err := reader.CommitMessages(ctx, message); err != nil && ctx.Err() == nil {
			c.logger.Warn("failed to commit notification source event", "error", err)
		}
	}
}
...
func (c *Consumer) processWithRetry(ctx context.Context, event domain.SourceEvent) error {
	...
	if err := c.publishDLQ(ctx, event, lastErr, c.retryConfig.MaxRetries); err != nil {
		c.logger.Warn("failed to publish notification DLQ event", "topic", event.Topic, "error", err)
		return lastErr
	}
	if c.metrics != nil {
		c.metrics.EventsDeadlettered.WithLabelValues(event.Topic).Inc()
	}
	c.logger.Warn("published notification event to DLQ", "topic", event.Topic, "error", lastErr)
	return lastErr
}
