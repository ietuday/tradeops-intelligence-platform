...
func TestProcessWithRetryPublishesDLQAfterExhaustion(t *testing.T) {
	writer := &fakeWriter{}
	processor := &fakeProcessor{err: errors.New("bad payload")}
	consumer := &Consumer{
		service:     processor,
	...
	err := consumer.processWithRetry(context.Background(), domain.SourceEvent{
		Topic:         "surveillance.alert.created",
		Key:           "event-1",
		Value:         []byte(`{"bad":true}`),
	...
	if event.OriginalTopic != "surveillance.alert.created" || event.ServiceName != "notification-service" || event.RetryCount != 2 || event.CorrelationID != "corr-1" {
