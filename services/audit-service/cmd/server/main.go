...
import (
	...
	"github.com/ietuday/tradeops-intelligence-platform/services/audit-service/internal/config"
	"github.com/ietuday/tradeops-intelligence-platform/services/audit-service/internal/db"
	httpapi "github.com/ietuday/tradeops-intelligence-platform/services/audit-service/internal/http"
	"github.com/ietuday/tradeops-intelligence-platform/services/audit-service/internal/kafka"
...
func main() {
	...
	defer stop()
	shutdownTracing, err := observability.SetupTracing(ctx, "audit-service")
	if err != nil {
	...
	auditService := service.NewAuditServiceWithPublisher(repository.NewAuditRepository(pool), metrics, producer, logger)
	consumer := kafka.NewConsumer(cfg.KafkaBrokers, cfg.KafkaTopics, cfg.KafkaGroupID, auditService, logger, metrics, kafka.RetryConfig{
	...
	go func() {
		logger.Info("audit service started", "port", cfg.Port)
