...
func NewConsumer(brokers []string, topics []string, groupID string, svc Processor, logger *slog.Logger, metrics *observability.Metrics, retryConfig RetryConfig) *Consumer {
	if groupID == "" {
	...
	return &Consumer{
		...
		logger:      logger,
		metrics:     metrics,
		dlqWriter:   newDLQWriter(brokers, "audit.dlq"),
...
func (c *Consumer) consume(ctx context.Context, reader readerCloser) {
	for {
		...
		if err != nil {
			...
			c.logger.Warn("failed to fetch audit source event", "error", err)
			continue
		}
...
func (c *Consumer) processWithRetry(ctx context.Context, event domain.SourceEvent) error {
	...
	if err := c.publishDLQ(ctx, event, lastErr, c.retryConfig.MaxRetries); err != nil {
		c.logger.Warn("failed to publish audit DLQ event", "topic", event.Topic, "error", err)
		return lastErr
	}
	if c.metrics != nil {
		c.metrics.EventsDeadlettered.WithLabelValues(event.Topic).Inc()
	}
	c.logger.Warn("published audit event to DLQ", "topic", event.Topic, "error", lastErr)
	return lastErr
}
