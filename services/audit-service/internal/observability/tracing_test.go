...
func TestRoutePatternWithChiContext(t *testing.T) {
	...
	var got string
	router.Get("/audit/{id}", func(w http.ResponseWriter, r *http.Request) {
		got = routePattern(r)
	})

	router.ServeHTTP(httptest.NewRecorder(), httptest.NewRequest(http.MethodGet, "/audit/123", nil))

	if got != "/audit/{id}" {
		t.Fatalf("routePattern with chi context = %q, want /audit/{id}", got)
	}
}

func TestTracingHandlersDoNotPanicWithoutChiRouteContext(t *testing.T) {
	handler := HTTPHandler("audit-service", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	handler.ServeHTTP(httptest.NewRecorder(), httptest.NewRequest(http.MethodGet, "/health", nil))

	traced := TraceAttributes("audit-service")(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
