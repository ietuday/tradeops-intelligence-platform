...
publish_event() {
  ...
  if command -v docker >/dev/null 2>&1 && docker compose -f "${COMPOSE_FILE}" ps "${REDPANDA_SERVICE}" >/dev/null 2>&1; then
    if node -e 'const fs = require("fs"); process.stdout.write(JSON.stringify(JSON.parse(fs.readFileSync(process.argv[1], "utf8"))) + "\n");' "${file}" |
...
print_manual_command() {
  ...
  cat <<EOF
Manual publish:
node -e 'const fs = require("fs"); process.stdout.write(JSON.stringify(JSON.parse(fs.readFileSync("${rel_file}", "utf8"))) + "\\n");' | \\
